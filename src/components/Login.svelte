<script lang="ts">
	...
	import { user, login } from '../firebase';
	type voidFunction = (id: number, price: number) => void;
	export let removeItem: voidFunction;
	export let addItem: voidFunction;
</script>
...
<main>
	{#if $user}
		<button
			...
			class="bg-nord8 p-1 rounded-md text-nord1">Logout</button
		>
	...
	{:else}
		<button
			on:click={login}
			class="bg-nord8 p-1 rounded-md text-nord1 hover:scale-110"
			style="transition:transform .2s">Log In</button
		>
	{/if}
</main>
